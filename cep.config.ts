...
const config: CEP_Config = {
  ...
  // GifsGuide — гифки для попапа-превью при наведении на блок в гайде (см.
  // GuideApp.tsx); копируется папкой целиком (не через Vite-импорт), потому
  // что состав файлов пополняется со временем и не должен требовать правок
  // кода на каждый новый файл.
  copyAssets: ["resources/mp4 110MB.aom", "resources/GifsGuide"],
  copyZipAssets: [],
};
